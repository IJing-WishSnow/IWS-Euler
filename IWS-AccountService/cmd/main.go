...
import (
	...
	"github.com/yourname/IWS-AccountService/consumer"
	"github.com/yourname/IWS-AccountService/service"
)
...
func main() {
	...
	log.Println("IWS-AccountService 启动")
	if err := settler.Run(ctx); err != nil {
		log.Fatalf("账户服务异常退出: %v", err)
	}
	log.Println("IWS-AccountService 已停止")
}

...
Kafka [orders topic]
  ...
  │     (撮合引擎)                     │
  │                           ├──► IWS-AccountService  (链下余额结算)
  │                           ├──► IWS-MarketData      (行情推送)
  │                           ├──► IWS-ChainClient     (链上合约结算)
...
- React 单页应用，提供交易界面
- Nginx 反向代理：`/api/*` → IWS-Gateway，`/ws` → IWS-MarketData
...
所有服务部署于 `iws-Euler` namespace，Helm chart 位于 `IWS-Deploy/`。
